...
if (bFind == 1) then 
	...
	if (IsMapOnGameSvr(mapid, dwGameSvrIP ) == 1) then 
		...
		if (bResult == 1) then 
			...
			msg = format("BOSS HOANG KIM %d, XUAT HIEN GameSvr %d DU LIEU KHOI DONG LIEN TIEP Relay, XUAT HIEN QUAI VAT, KHU VUC %d, ID Npc %d, DANG CAP Npc%d.", Sid, dwGameSvrIP, mapid, npctid, npclevel);
			OutputMsg(msg);
		else
...
function TaskShedule()
	...
	--删除从前的数据
	success = ClearRecordOnShareDB("GoldBoss",Sid, 0,1,0);
	OutputMsg("XOA DATABASE BOSS VANG "..Sid);
end

function TaskContent()
	bFind , key1, key2 = GetFirstRecordFromSDB("GoldBoss",Sid, 0, 1, 0);
	if (bFind == 1) then
		OutputMsg("TIM THAY BOSS O database".. Sid..","..key2.."THONG TIN CHUONG TRINH");
		bResult , npctid, npclevel, npcstate = GetCustomDataFromSDB("GoldBoss", Sid, key2, "iii");
		msg =	format("Du lieu %d trong chuong trinh co boss vang Npc: %d, Cap :%d, Trang Thai: %d", Sid, npctid, npclevel, npcstate);
		OutputMsg(msg);
		if (bResult == 1) then 
