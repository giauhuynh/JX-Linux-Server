	...
	-- citywar_head.lua Edited By Bil4i3n
	
	_bilTbIsLimitOpenDay = {
...
function bilCheckIsLimitOpenCityWar(bilType)
	-- bilType: 1 - Register, 2 - CityWar
	local bil_1 = tonumber(date("%w"))
	local bil_2 = getn(_bilTbIsLimitOpenDay)
	for iCheck = 1, bil_2 do
		if _bilTbIsLimitOpenDay[iCheck][bilType] == bil_1 then
			return 1
		end
	end
	if bilType == 1 then
		print(format("Hom nay khong cho phep _DANG KY_ CTC - W: %d", bil_1))
	elseif bilType == 2 then
		print(format("Hom nay khong cho phep CTC - W: %d", bil_1))
	end
	return 0
end
	...
	function cw_startsignup_fun(nweek,ncan)
		if (tonumber(date( "%w ")) == nweek and cw_CanStart(ncan,1) == 1) then
			...
			LG_ApplySetLeagueTask(LEAGUETYPE_CITYWAR_SIGN, Bil_LG_CN_TB_CITYWAR_ARRANGE[ncan][3], 1, 1);
			...
			OutputMsg(format( "Cong Thanh Chien %s bat dau bao danh",BIL_S3RL_TB_CITYWAR_ARRANGE[ncan][3]))
		end;
	end
	...
	function cw_endsignup_fun(nweek,ncan)
		LG_ApplySetLeagueTask(LEAGUETYPE_CITYWAR_SIGN, Bil_LG_CN_TB_CITYWAR_ARRANGE[ncan][3], 1, 0);
		local tbVotes = citywar_tbLadder:Bil_GetInfo();
		if getn(tbVotes) == 1 then
			Bil_NameFirstTong = tbVotes[1].szName;
			Bil_NameDoubleTong = "";
		end
		if getn(tbVotes) >= 2 then
			Bil_NameFirstTong = tbVotes[1].szName;
			Bil_NameDoubleTong = tbVotes[2].szName;
		end
		local Bil_szWarCityName = Bil_LG_CN_TB_CITYWAR_ARRANGE[ncan][3];
		if (tonumber(date( "%w ")) == nweek and cw_CanStart(ncan,2) == 1) then
			local nlid = LG_GetLeagueObj(LEAGUETYPE_CITYWAR_FIRST, Bil_LG_CN_TB_CITYWAR_ARRANGE[ncan][3]);
			szWarCityName = TB_CITYWAR_ARRANGE[ncan][3]
			if (FALSE(nlid)) then
				OutputMsg(format( "Cong thanh chien %s khong co bang hoi tham gia bao danh, tuan nay van su thai binh.",BIL_S3RL_TB_CITYWAR_ARRANGE[ncan][3]));
			...
			local szFirstTong = GetRandomChallenger(Bil_LG_CN_TB_CITYWAR_ARRANGE[ncan][3]);
			...
			if (GetCityOwner(ncan) == "" or GetCityOwner(ncan) == nil) then
				if getn(tbVotes) == 1 then
					GlobalExecute(format( "dwf \\script\\missions\\citywar_global\\citywar_function.lua citywar_appointviceroy([[%s]],[[%s]])",Bil_szWarCityName,Bil_NameFirstTong));
					--nÕu chØ cã 1 bang ®¨ng ký trong lÇn ®Çu c«ng thµnh th× sÏ Set bang ®ã chiÕm thµnh.
					zzMsg2SubWorld = "Thµnh <color=green>"..szWarCityName.."<color> lÇn ®Çu tiªn c«ng thµnh chØ cã bang <color=yellow>"..Bil_NameFirstTong.."<color> ®¨ng ký, v× vËy thµnh <color=green>"..szWarCityName.."<color> sÏ do bang <color=yellow>"..Bil_NameFirstTong.."<color> qu¶n lý."
					zzAddLocalNews = "Thµnh "..szWarCityName.." lÇn ®Çu tiªn c«ng thµnh chØ cã bang "..Bil_NameFirstTong.." ®¨ng ký, v× vËy thµnh "..szWarCityName.." sÏ do bang "..Bil_NameFirstTong.." qu¶n lý."
					GlobalExecute(format( "dw Msg2SubWorld([[%s]])", zzMsg2SubWorld));
					GlobalExecute(format( "dw AddLocalNews([[%s]])",zzAddLocalNews));
					OutputMsg(format( "Thanh %s se do bang %s lam chu.",BIL_S3RL_TB_CITYWAR_ARRANGE[ncan][3],Bil_NameFirstTong));
				end
				if getn(tbVotes) >= 2 then
					GlobalExecute(format( "dwf \\script\\missions\\citywar_global\\citywar_function.lua citywar_appointviceroy([[%s]],[[%s]])",Bil_szWarCityName,Bil_NameFirstTong));
					...
					GlobalExecute(format( "dwf \\script\\missions\\citywar_global\\citywar_function.lua citywar_appointchallenger([[%s]],[[%s]])",Bil_szWarCityName,Bil_NameDoubleTong));
					zzMsg2SubWorld = "<color=green>C«ng thµnh chiÕn<color> <color=yellow>"..szWarCityName.."<color> Top 1 Khiªu chiÕn lÖnh bang thñ thµnh "..Bil_NameFirstTong..", Top 2 khiªu chiÕn lÖnh bang "..Bil_NameDoubleTong.." sÏ c«ng thµnh vµo ngµy mai."
					zzAddLocalNews = "C«ng thµnh chiÕn "..szWarCityName.." Top 1 Khiªu chiÕn lÖnh bang thñ thµnh "..Bil_NameFirstTong..", Top 2 khiªu chiÕn lÖnh bang "..Bil_NameDoubleTong.." sÏ c«ng thµnh vµo ngµy mai."
					GlobalExecute(format( "dw Msg2SubWorld([[%s]])", zzMsg2SubWorld))
					GlobalExecute(format( "dw AddLocalNews([[%s]])",zzAddLocalNews));
					OutputMsg(format( "Thanh %s Bang thu thanh la %s Bang cong thanh cho ngay mai la %s.",BIL_S3RL_TB_CITYWAR_ARRANGE[ncan][3],Bil_NameFirstTong,Bil_NameDoubleTong));
				end
			else
				GlobalExecute(format( "dwf \\script\\missions\\citywar_global\\citywar_function.lua citywar_appointchallenger([[%s]],[[%s]])",Bil_szWarCityName,szFirstTong));
				OutputMsg(format( "%s tranh doat lenh bai thanh cong, tro thanh khieu chien bang hoi %s vao ngay mai.",szFirstTong,BIL_S3RL_TB_CITYWAR_ARRANGE[ncan][3]))
