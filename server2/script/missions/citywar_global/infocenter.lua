...
function main()
	if bilNPCCongThanhQuan ~= 1 then
		return Talk(1, "", "<color=Orange>C«ng thµnh Quan: <color>H«m nay t¹i h¹ bÞ c¶m phong hµn, h«m sau c¸c h¹ l¹i ®Õn vËy!")	
	end
	ArenaMain()
end
...
function citywar_CheckVotes()
	...
	local szMsg = format("<dec><npc>Bªn d­íi lµ bang héi tham gia ®Êu gi¸ khiªu chiÕn lÖnh thµnh <color=green>%s<color>: <enter>%s%s%s<enter>", Bil_GetNameCityWithnnCan1To7(nCityId), strfill_center("<color=yellow>STT<color>",4, " "), strfill_center("<color=yellow>       Bang héi<color>", 20, " "), strfill_center("<color=yellow>           Sè l­îng<color>", 20, " "))
	local res = {}
...
function ArenaMain()
	local nCityId = getSigningUpCity(1);
	local Bil_GoCityWarCityID = Bil_WhichWarBegin();
	if (Bil_GoCityWarCityID >= 1 and Bil_GoCityWarCityID <= 7) then
		Bil_GoCityWarTong1, Bil_GoCityWarTong2 = GetCityWarBothSides(Bil_GoCityWarCityID);
		return Say("HiÖn t¹i <color=green>"..Bil_GetNameCityWithnnCan1To7(Bil_GoCityWarCityID).."<color> ®ang tiÕn hµnh c«ng thµnh chiÕn,<enter>Bang thñ thµnh lµ: <color=yellow>"..Bil_GoCityWarTong2.."<color><enter>Bang c«ng thµnh lµ: <color=yellow>"..Bil_GoCityWarTong1.."<color>.<enter><enter>VÞ nghÜa sÜ cã muèn tham gia kh«ng?", 8,
			"§i ChiÕn tr­êng c«ng thµnh/Bil_GoCityWarMap",
		...
		if (tonumber(GetLocalDate("%H"))>= 18 and tonumber(GetLocalDate("%H")) < 19 and getSignUpState(nCityId) == 1) then
			return Say(format("HiÖn t¹i c«ng thµnh chiÕn thµnh <color=green>%s<color> ®ang cho b¸o danh, ng­¬i muèn ®¨ng ký kh«ng?",Bil_GetNameCityWithnnCan1To7(nCityId)), 7, 
...
function Bil_GoCityWarMap()
	...
	if (Bil_GoCityWarCityID >= 1 and Bil_GoCityWarCityID <= 7) then
		...
		Tong1, Tong2 = GetCityWarBothSides(Bil_GoCityWarCityID);
		str = format("HiÖn t¹i <color=green>"..Bil_GetNameCityWithnnCan1To7(Bil_GoCityWarCityID).."<color> ®ang tiÕn hµnh c«ng thµnh chiÕn,<enter>Bang thñ thµnh lµ: <color=yellow>%s<color><enter>Bang c«ng thµnh lµ: <color=yellow>%s<color>.<enter><enter>VÞ nghÜa sÜ cã muèn tham gia Bªn nµo?", Tong2, Tong1);
...
function GiveTiaoZhanLing()
	...
	GiveItemUI("Giao nép khiªu chiÕn lÖnh", "Khiªu chiÕn lÖnh cã thÓ ®æi 20000 ®iÓm kinh nghiÖm, dïng ®Ó b¸o danh c«ng thµnh chiÕn cho bang héi.", "sure_GiveTiaoZhanLing", "OnCancel");
end
...
function sure_GiveTiaoZhanLing(nCount)
	...
	for i = 1, nCount do
		...
		bilCountKCL = bilCountKCL + GetItemStackCount(nItemidx)
	end;
	local nDate = tonumber(tonumber(GetLocalDate("%y"))..tonumber(GetLocalDate("%m"))..tonumber(GetLocalDate("%d")));
	local nLibao = GetTask(TIAOZHANLING_TASK_DATE);
	local nOlddate = tonumber(GetByte(nLibao,1)..GetByte(nLibao,2)..GetByte(nLibao,3));
	local nCountall = GetTask(TIAOZHANLING_TASK_COUNT);
	if ( nOlddate == nDate and nCountall+bilCountKCL > 300) then
	...
	SetTask(TIAOZHANLING_TASK_COUNT,nCountall+bilCountKCL);
	LG_ApplyAppendMemberTask(TIAOZHANLING_LGTYPE,TIAOZHANLING_LGName, szTongName, LGTSK_TIAOZHANLING_COUNT, bilCountKCL, "", "");
	-- nAddExp = bilCountKCL * 50000
	nAddExp = bilCountKCL * 20000
	AddOwnExp(nAddExp)
	Msg2Player(format("B¹n ®· nép vµo <color=green>%d<color> khiªu chiÕn lÖnh, nhËn ®­îc <color=yellow>%d<color> ®iÓm kinh nghiÖm",bilCountKCL,nAddExp))
	Bil_SetTask(19, Bil_GetTask(19) + bilCountKCL)
	Msg2Player("Tæng sè khiªu chiÕn lÖnh c¸c h¹ ®· nép vµo cho bang lµ: <color=yellow>"..Bil_GetTask(19))
	WriteLog(format("[C«ng thµnh chiÕn_giao khiªu chiÕn lÖnh]Date:%s Account:%s Name:%s Tong:%s Count:%d Exp:%d",GetLocalDate("%y-%m-%d %H:%M:%S"),GetAccount(),GetName(),szTongName,bilCountKCL,nAddExp))
end;
	...
	if (GetName() == GetTongMaster()) then
		...
		for i = 1, 7 do
			...
			if (Tong1 == TongName) then
				...
				str_format = format("Th× ra ®¹i hiÖp lµ ng­êi khiªu chiÕn thµnh <color=green>%s<color>, lîi h¹i qu¸, ë ®©y cã b¸n lÖnh bµi c«ng thµnh cã hiÖu lùc trong 5 ngµy dµnh liªn minh cña quÝ bang, mçi lÖnh bµi gi¸ <color=yellow>%s l­îng.",Bil_GetNameCityWithnnCan1To7(i),CardPrice);
				Say(str_format, 2, "Mua mét Ýt/DealBuyCard", "T¹m thêi kh«ng cÇn/OnCancel");
				return
			elseif (Tong2 == TongName) then
				SetTaskTemp(15, CardTab[i * 2]);
				str_format = format("Th× ra ®¹i hiÖp lµ th¸i thó thµnh <color=green>%s<color>, lîi h¹i qu¸, t¹i ®©y cã b¸n lÖnh bµi thñ thµnh cã hiÖu lùc 5 ngµy dµnh cho liªn minh cña quÝ bang, mçi lÖnh bµi gi¸ <color=yellow>%s l­îng.",Bil_GetNameCityWithnnCan1To7(i),CardPrice);
				Say(str_format, 2, "Mua mét Ýt/DealBuyCard", "T¹m thêi kh«ng cÇn/OnCancel");
				return
			end;
		end;
...
function GameInfo()
Say("Muèn t×m hiÓu th«ng tin thµnh thÞ nµo?", 9, Bil_GetNameCityWithnnCan1To7(1).."/CityInfo", Bil_GetNameCityWithnnCan1To7(2).."/CityInfo", Bil_GetNameCityWithnnCan1To7(3).."/CityInfo", Bil_GetNameCityWithnnCan1To7(4).."/CityInfo", Bil_GetNameCityWithnnCan1To7(5).."/CityInfo", Bil_GetNameCityWithnnCan1To7(6).."/CityInfo", Bil_GetNameCityWithnnCan1To7(7).."/CityInfo", "Trë vÒ/ArenaMain", "Kh«ng cÇn ®©u/OnCancel");
end;
...
function CityInfo(nSel)
	...
	Say(format("Muèn t×m hiÓu th«ng tin g× vÒ c«ng thµnh chiÕn <color=green>%s<color>?",Bil_GetNameCityWithnnCan1To7(nCityId)), 4, 
...
function RegisterInfo()
	...
	if (nCityId ~= getSigningUpCity(1) or getSignUpState(nCityId) ~= 1) then
		Say(format("HiÖn t¹i c«ng thµnh chiÕn <color=green>%s<color> kh«ng ë giai ®o¹n b¸o danh.",Bil_GetNameCityWithnnCan1To7(nCityId)), 2, "Trë vÒ/GameInfo", "Kh«ng cÇn ®©u/OnCancel");
		return 0;
	end;
...
Say(format("C«ng thµnh chiÕn <color=green>%s<color> ®ang ®­îc chuÈn bÞ, bang héi thi ®ua lÖnh bµi xÕp h¹ng thø nhÊt lµ : <color=yellow>%s<color><color=red><enter>NÕu cã bang nµo cã sè l­îng lÖnh bµi b»ng víi bang xÕp thø 1, th× hÖ thèng sÏ ngÉu nhiªn chän ra bang c«ng thµnh cho ngµy mai.<color>",Bil_GetNameCityWithnnCan1To7(nCityId),szElector), 2, "Trë vÒ/GameInfo", "Kh«ng cÇn ®©u/OnCancel");
end;
...
function CityWarInfo()
	...
	local str_format = format("C«ng thµnh chiÕn <color=green>%s<color> ngµy mai: ",Bil_GetNameCityWithnnCan1To7(nCityId));
	local str1, str2 = GetCityWarBothSides(nCityId);
	if (str1 ~= "" and str2 ~= "" ) then
		if (nCityId == getSigningUpCity(2)) then
			str_format = format("C«ng thµnh chiÕn <color=green>%s<color> h«m nay: ",Bil_GetNameCityWithnnCan1To7(nCityId));
			if (HaveBeginWar(nCityId) == 1) then 
				str_format = format("Tr­íc m¾t <color=green>%s<color> ®ang ë giai ®o¹n c«ng thµnh chiÕn: ",Bil_GetNameCityWithnnCan1To7(nCityId));
			end;
		end;
		...
		Say(format("Tr­íc m¾t <color=green>%s<color> ch­a b­íc vµo giai ®o¹n c«ng thµnh chiÕn!",Bil_GetNameCityWithnnCan1To7(nCityId)) , 2, "Trë vÒ/GameInfo", "Kh«ng cÇn ®©u/OnCancel");
	end;
end;
...
function SignUpCityWar()
	...
	local szMsg = format("<dec>HiÖn t¹i ®ang tiÕn hµnh b¸o danh thµnh <color=green>%s<color>.",Bil_GetNameCityWithnnCan1To7(nCityId));
...
function checkSignUpCityWar(szTongName, nTongID, nCityId)
	...
	elseif (checkCityOwner(szTongName) ~= 0) then
		Say("Ng­êi ®· lµ chñ thµnh <color=yellow>"..Bil_GetNameCityWithnnCan1To7(checkCityOwner(szTongName)).."<color>, h·y lo gi÷ thµnh cña ngµi tr­íc ®·.", 0);
	elseif (checkCItyChallenger(szTongName) ~= 0) then
			Say(format("Ng­¬i ®· lµ phe khiªu chiÕn thµnh <color=green>%s<color>, kh«ng thÓ tranh ®ua lÖnh bµi trong ngµy h«m nay.",Bil_GetNameCityWithnnCan1To7(nCityId)));
