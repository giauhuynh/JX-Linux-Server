...
--Ö÷¶Ô»°¿ò
function main()
	if bilPNCVoLamTruyenNhan ~= 1 then
		return Talk(1, "", "<color=Orange>Vâ L©m TruyÒn Nh©n: <color>H«m nay t¹i h¹ bŞ c¶m phong hµn, h«m sau c¸c h¹ l¹i ®Õn vËy!")	
	end
	...
	local n_next = GetGlbValue(GLB_WLLS_NEXT)
	local Bil_str = "<color=Orange>Vâ l©m truyÒn nh©n:<color> "..WLLS_TAB[n_next].text_main[n_level]
	...
	wlls_descript(Bil_str, wlls_add_option(tb_option, "Ta chØ tiÖn ghĞ qua"))
end
...
function wlls_want2signmap()
	...
	if (n_matchphase < 2) then
		...
		Say("<color=Orange>Vâ L©m truyÒn Nh©n: <color>Hiªn t¹i lµ thêi gian liªn ®Êu t¹m nghØ, héi tr­êng sÏ t¹m ®ãng cöa, b¹n h·y chê ®Õn kú thi ®Êu h·y quay l¹i!", 0)
		return 
	end
	
	if GetLevel() < WLLS_LEVEL_JUNIOR then
		return Say("<color=Orange>Vâ L©m truyÒn Nh©n: <color><color=red>Vâ l©m liªn ®Êu<color> vÉn lµ n¬i Anh   hïng thiªn h¹ luËn kiÕm, ®¼ng cÊp cña ng­¬i vÉn ch­a ®ñ <color=yellow>"..WLLS_LEVEL_JUNIOR.." cÊp<color>, anh hïng kh¾p n¬i tÒ tô, ngäa hæ tµng longe r»ng ng­¬i chØ lµ kÎ hËu bèi! Tèt h¬n nªn luyÖn tËpthªm!")
	end
	...
	local n_group = 1 --Bil Set tÊt c¶ c¸c nhãm tËp trung l¹i 1 chç-> §¹i l? cao cÊp - BiÖn kinh s¬ cÊp
	...
	--´«ËÍµ½±¨ÃûµãµØÍ¼
	local Bil_MapType = "<color=green>B¸o danh<color>";
	if n_level == 1 then
		-- Bil_MapType = "<color=green>80 -> 119<color>";
	elseif n_level == 2 then
		-- Bil_MapType = "<color=pink>Cao CÊp<color>";
	end
	local n_signmap = wlls_get_mapid(1, n_mtype, n_group)
	NewWorld(n_signmap, WLLS_MAPPOS_SIGN[1], WLLS_MAPPOS_SIGN[2])
	Msg2Player("B¹n ®· ®Õn khu vùc thi ®Êu <enter>           "..Bil_MapType.." - <color=yellow>"..wlls_get_desc(3, n_mtype, n_group).."<color>")
...
function wlls_buy_honour()
	...
	Talk(1, "Bil_BuyiTemWithHonour", "<enter><color=Orange>"..GetName().."<color> "..SexStr.." hiÖn ®ang cã<color=yellow> "..GetTask(WLLS_TASKID_HONOUR).."<color> ®iÓm vinh dù");
	function Bil_BuyiTemWithHonour()
