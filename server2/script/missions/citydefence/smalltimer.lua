...
function OnTimer()
	...
	if ( t == RUNGAME_TIME ) then	--¿ªÕ½Ê±¼äµ½£¬ÌáÊ¾ÊØ³Ç¿ªÊ¼£¬²¢Ë¢µÚÒ»Åú¹Ö
		...
		AddGlobalNews("ChiÕn dÞch thñ thµnh ®· b¾t ®Çu, "..tbDEFENCE_ANNOUNCE[citycamp])
		Msg2MSAll( MISSIONID, "<color=yellow>Qu©n ®Þch ®· b¾t ®Çu c«ng thµnh, v× ®Ó b¶o vÖ thµnh chóng ta c¸c T­íng sÜ h·y x«ng lªn!" )
Msg2SubWorld("ChÕn Tr­êng<color=green> Phong Háa Liªn Thµnh b¾t ®Çu khai chiÕn anh em nghÜa sÜ cè thñ 3 cæng kh«ng cho ®Þch tÊn c«ng.")
	...
	if ( t > RUNGAME_TIME ) then
		...
		if ( mod( t, 18 ) == 0 ) then	--±¨ÃûÊ±¼äÖÐ£¬Ã»6·ÖÖÓÌáÊ¾±¨Ãû½áÊøÊ±¼ä£¬²¢·¢¹ö¶¯ÏûÏ¢
			...
			if ( RestMin > 0 ) then
				...
				AddGlobalNews("Phong Háa Liªn Thµnh ®· nhËn b¸o danh, cã muèn tham gia kh«ng?"..tbDEFENCE_ANNOUNCE[citycamp].."Thêi gian khai chiÕn chØ cßn l¹i"..RestMin.."phót")
			end
		end
	end
end
...
function cd_sendmsg2global()
	local szMsg = "Thñ thµnh Phong Háa liªn thµnh"..GetMissionS(MS_S_CD_NAME).."ChiÕn tranh ®ang trong giai ®o¹n ¸c liÖt."..tbDEFENCE_ANNOUNCE[GetMissionV(MS_CITYCAMP)]
	AddGlobalNews(szMsg)
end