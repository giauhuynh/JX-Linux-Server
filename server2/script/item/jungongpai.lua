...
function main(nItem)	
	...
	local _,_,detail = GetItemProp(nItem)
	...
	for i = 2,getn(JUNGONGPAI_exp) do
		...
		if num < sum then
			...
			AddOwnExp(a)
			Msg2Player(format("Sö dông Hu©n c«ng bµi Tèng Kim nhËn ®­îc %d ®iÓm kinh nghiÖm.",a))
			WriteLog(format("[Hu©n c«ng bµi Tèng Kim]\t date:%s \t Account:%s \t Name:%s \t Sö dông Hu©n c«ng bµi Tèng Kim nhËn ®­îc %d ®iÓm kinh nghiÖm \t	",GetLocalDate("%Y-%m-%d %H:%M:%S"),GetAccount(),GetName(),a));
			break;
		end
	end
end
