...
function main(nItemIdx)
	...
	elseif (TB_XINNIAN_GIFE[detail][3] == 3) then	--µÚ3¸ö²ÎÊý£¨ÏêÏ¸ÀàÐÍ£©°´Ìõ¼þ
		...
		if (CalcFreeItemCellCount() < TB_XINNIAN_GIFE[detail][5]) and (detail == 910) then
			Msg2Player("Chç trèng trong hµnh trang cña b¹n kh«ng ®ñ: "..TB_XINNIAN_GIFE[detail][5]);
			return 1;
		end;
		
		for i = TB_XINNIAN_GIFE[detail][4],	TB_XINNIAN_GIFE[detail][5] do
			if detail == 910 then
				-- a = AddItem(tbGift[1],	tbGift[2],	i,	tbGift[4],	tbGift[5],	tbGift[6])
				a = AddItemNoStack(tbGift[1],	tbGift[2],	i,	tbGift[4],	tbGift[5],	tbGift[6])
				SetItemBindState(a, -2)
			else
