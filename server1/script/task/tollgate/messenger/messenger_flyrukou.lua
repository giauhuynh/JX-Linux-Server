...
function main()
	...
	-- end
	 ...
	 Say("<color=Orange>DŞch Quan: <color>: §· b¾t ®Çu tİnh thêi gian qua ¶i, cè g¾ng lªn! B©y giê b¹n muèn lµm g×?",getn(tbDialog), unpack(tbDialog))
end

function fly_starttask()
	if GetTask(1212) >= _BIL_TIME_EVENT_ON_PLAYER then
		return Talk(1, "", "<color=Orange>DŞch Quan: <color> Thêi gian lµm nhiÖm vô cña "..GetName().." ®¹i nh©n ®· hÕt, cung kİnh ®­a ngµi vÒ BiÖn Kinh phñ <pic=107>")
	end
	if ( nt_getTask(1201) == 10 ) then
		...
		SetPos(1559,3113);
		Msg2Player("§· b¾t ®Çu tİnh thêi gian lµm nhiÖm vô ")
		Msg2Player("Thêi gian Phong Kú cßn l¹i cña ®¹i nh©n lµ: <color=yellow>"..floor((_BIL_TIME_EVENT_ON_PLAYER/60) - (GetTask(1212)/60)).."<color> Phót")
...
function fly_movecity()
	...
	Msg2Player("Phong Kú DŞch tr¹m cung tèng <color=yellow>"..GetName().."<color> §¹i nh©n ®Õn <color=yellow>BiÖn Kinh<color> DŞch tr¹m!")
end
