...
ContentList = {
	"<#> NÕu ai cã ®ñ 160  s¸t thñ gi¶n trong tay th× giÊc méng trë thµnh ®Ö nhÊt s¸t thñ ®· xem nh­ hoµn thµnh. Mçi ngµy chØ cÇn hoµn thµnh <color=yellow>8 nhiÖm vô<color> lµ ®­îc.<enter>H·y tiÕp nhËn nhiÖm vô “th¸ch thøc thêi gian” ®Ó thÓ nghiÖm søc m¹nh ®ång ®éi vµ kh¶ n¨ng s¸t thñ cña m×nh.<enter>",
	...
	"<#> Ta kh«ng muèn giÕt ng­êi ®©u, hñy bá nhiÖm vô./cancel",	--10
	...
	"<#> B¹n ®· hîp thµnh mét s¸t thñ gi¶n Thuéc tİnh <color=",
	"<#> <color>, s¸t thñ gi¶n lµ mét s¸t thñ phi phµm. B¹n cã thÓ dïng mét s¸t thñ cïng cÊp ®Ó so tµi víi s¸t thñ gi¶n, quy t¾c th¾ng thua ®­îc ¸p dông theo quy t¾c t­¬ng kh¾c cña ngò hµnh. ",
...
function main()
	if bilNPCNhipThiTran ~= 1 then
		return Talk(1, "", "<color=Orange>Nhİp Thİ TrÇn: <color>H«m nay t¹i h¹ bŞ c¶m phong hµn, h«m sau c¸c h¹ l¹i ®Õn vËy!")	
	end
	...
	local tbDialog = {ContentList[24],ContentList[17],ContentList[2],ContentList[3],ContentList[4],ContentList[5],ContentList[6],ContentList[7],ContentList[8],ContentList[9],ContentList[10],ContentList[11]};
	...
	Describe(DescLink_NieShiChen.."NÕu ai cã ®ñ 160  s¸t thñ gi¶n trong tay th× giÊc méng trë thµnh ®Ö nhÊt s¸t thñ ®· xem nh­ hoµn thµnh. Mçi ngµy chØ cÇn hoµn thµnh <color=yellow>8 nhiÖm vô<color> lµ ®­îc.<enter>H·y tiÕp nhËn nhiÖm vô “th¸ch thøc thêi gian” ®Ó thÓ nghiÖm søc m¹nh ®ång ®éi vµ kh¶ n¨ng s¸t thñ cña m×nh.<enter><enter>Tæng sè s¸t thñ ®· bŞ ng­¬i h¹ thñ h«m nay: <color=yellow>"..GetTask(1193).." <color>ng­êi", getn(tbDialog), unpack(tbDialog));
end
...
function givetask(taskid)
	...
	if bilNewWorldPlayer2PosBoss == 1 then
...
function Bil_Go2BossPos(taskid)
	local Bil_PosW = killertabfile:getCell("BilBossPosW",taskid)
	...
	local BilProcessX, BilProcessY = Bil_ProcessBossPos(Bil_PosX,Bil_PosY)
	local Bil_ChNW = NewWorld(Bil_PosW, BilProcessX, BilProcessY)
	if Bil_ChNW == 0 or Bil_ChNW == nil then
		Msg2Player(Bil_PosW..BilProcessX..BilProcessY.." Lçi Map!")
	else
		SetFightState(1)
		Msg2Player("H·y h¹ thñ v« t×nh cho ta! <pic=81>")
	end
end
...
function compose()
	...
	GiveItemUI("Giao diÖn hîp thµnh s¸t thñ gi¶n","Yªu cÇu:<enter><color=blue>5 S¸t thñ lÖnh","bilexchange_token", "no")
end

function bilexchange_token(bilCount)
	if bilCount ~= 5 then
		return Say("Ta chØ yªu cÇu 5 s¸t thñ lÖnh, kh«ng h¬n kh«ng kĞm!", 0)
	end
	local bil_2 = {}
	for bil_1_1 = 1, bilCount do
		local bil_1_2 = GetGiveItemUnit(bil_1_1)
		local bil_1_G, bil_1_D, bil_1_P, bil_1_L, bil_1_S = GetItemProp(bil_1_2)
		if bil_1_G ~= 6 and bil_1_P ~= 339 then
			return Say("H×nh nh­ nguyªn liÖu mµ ®¹i hiÖp ch­a cho t¹i h¹ cã c¸i kh«ng ph¶i lµ s¸t thñ lÖnh, vui lßng kiÓm tra l¹i nguyªn liÖu!", 0)
		end
		tinsert(bil_2, bil_1_2)
	end
	for bil_2_1 = 1, getn(bil_2) do
		RemoveItemByIndex(bil_2[bil_2_1])
	end
	AddItem( 6, 1, 400, 90, random(0, 4), 0)
	return Say("NhËn ®­îc 1 S¸t thñ gi¶n!", 0)
end
...
function killerCoundTakedTask(nLowLevel, nHighLevel)
	...
	if (myDate == nDate and nt_getTask(TSKID_KILLERMAXCOUNT) >= KILLER_MAXCOUNT) then
		Describe(DescLink_NieShiChen.." S¸t thñ cã mét tè chÊt rÊt quan träng gäi lµ khinh kÎ b¹i trËn.  H«m nay ng­¬i ®· h¹ thñ "..KILLER_MAXCOUNT.."  ng­êi råi, ngµy mai h·y quay l¹i.", 1, ContentList[15]);
		return 0;
