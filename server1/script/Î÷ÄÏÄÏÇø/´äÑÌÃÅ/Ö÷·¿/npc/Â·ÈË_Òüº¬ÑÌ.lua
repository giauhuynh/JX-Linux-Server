...
function oldentence()
	...
	if GetTask(5995) == 6 then -- thuy yen
	...
	-------------------------------------------------------------------------
	if bilFunc_ChuongMonThapDaiPhai(6, GetLevel(), GetLastFactionNumber(), GetFaction(), GetName()) then
	
		local a, b = bilFunc_ChuongMonThapDaiPhai(6, GetLevel(), GetLastFactionNumber(), GetFaction(), GetName())
	...
	tinsert(tbDes, "§Ö tö muèn b¸i nghÖ ®Çu s­ vµo Thóy yªn m«n!/#bilPartner2Sect(6)")
...
function bil4i3nXS_Return()
	local _, nTongID = GetTong();
	if GetTask(5995) ~= 6 and nTongID ~= 0 then ----------
		return Talk(1,"","Ng­êi cña gi¸o ph¸i kh¸c t¹i sao l¹i ®Õn ®©y nhê cËy ta?")
	end
	if(GetCash() >= 50000) then						
		Msg2Faction("cuiyan", GetName().." tõ h«m nay trë vÒ l¹i s­ m«n, mong c¸c huynh ®Ö chØ gi¸o!", GetName()) ----------
		Talk(1,"","RÊt tèt! Hoan nghªnh ng­¬i trë l¹i Thóy yªn m«n!")
		Pay(50000)
		SetTask(5995, 0)
		SetFaction("cuiyan") ------------------
		SetCamp(3) -------------------
		SetCurCamp(3) ----------------------
		-- SetRank(67) -------------------
		AddNote("§· quay trë l¹i Thóy yªn m«n. ")
	else
...
function bil4i3nXS()
	...
	Msg2Faction("cuiyan", GetName().." tõ h«m nay rêi khái s­ m«n, xuèng nói hµnh tÈu giang hå, mong c¸c huynh ®Ö chØ gi¸o!", GetName()) -----------
	Talk(1,"","Tèt l¾m! Hµnh tÈu giang hå nhí gióp ®ì nh÷ng ng­êi khèn khã!")
	SetTask(5995, 6) ------------
	AddNote("Rêi khái Thóy yªn m«n, tiÕp tôc hµnh tÈu giang hå. ") -------------
	Msg2Player("B¹n ®· rêi khái Thóy yªn m«n, tiÕp tôc hµnh tÈu giang hå. ") -------------
	SetFaction("")
	-- SetFaction("cuiyan")
	SetCamp(4)
	SetCurCamp(4)
...
function Uworld126_finish()
	if ((GetTask(6) >= 70*256) and (GetTask(6) ~= 75*256)) or
		(GetTask(5893) == 1) then
