	...
	if (myTaskType == 4) then
		...
		if (nt_getTask(1031) == myMapID) then
			...
			if (GetByte(nt_getTask(1032),1) == 1) then
				...
				Msg2Player(bil.C(10, "B¹n nhËn ®­îc mét tÊm"..myMapName.."§Şa §å chİ! HiÖn t¹i b¹n cã tæng céng "..myMapNum.." tÊm."));
		...
		Msg2Player(bil.C(10, "B¹n nhËn ®­îc mét m¶nh b¶n ®å S¬n Hµ X· T¾c! HiÖn t¹i b¹n cã tæng céng "..nWorldMaps.." m¶nh b¶n ®å S¬n Hµ X· T¾c."));
