...
function main()
	...
	-- if GetTask(5995) == 5 then -- Nga my
		-- tbDailog:AddOptEntry("§Ö tö muèn vÒ l¹i s­ m«n", bil4i3nXS_Return)
	-- else
		-- tbDailog:AddOptEntry("§Ö tö muèn xuÊt s­, xuèng nói hµnh tÈu giang hå", bil4i3nXS)
	-- end
	
	-- tbDailog:AddOptEntry("§Ö tö muèn b¸i nghÖ ®Çu s­ vµo Nga my!", bilPartner2Sect, {5})
	...
	if GetTask(5995) == 5 then -- Nga my
		tinsert(tbDes, "§Ö tö muèn vÒ l¹i s­ m«n/bil4i3nXS_Return")
	else
		tinsert(tbDes, "§Ö tö muèn xuÊt s­, xuèng nói hµnh tÈu giang hå/bil4i3nXS")
	end
	
	-------------------------------------------------------------------------
	if bilFunc_ChuongMonThapDaiPhai(5, GetLevel(), GetLastFactionNumber(), GetFaction(), GetName()) then
	
		local a, b = bilFunc_ChuongMonThapDaiPhai(5, GetLevel(), GetLastFactionNumber(), GetFaction(), GetName())
	...
	tinsert(tbDes, "§Ö tö muèn b¸i nghÖ ®Çu s­ vµo Nga my ph¸i!!/#bilPartner2Sect(5)")
	tinsert(tbDes, "Muèn thØnh gi¸o viÖc kh¸c!/common_talk")
	
	Say("GÇn ®©y ta cã rÊt nhiÒu viÖc gi¶i quyÕt, ng­¬i ®Õn ®©y cã viÖc g×.", getn(tbDes), tbDes)
end

function bil4i3nXS_Return()
	local _, nTongID = GetTong();
	if GetTask(5995) ~= 5 and nTongID ~= 0 then ---------- 
		return Talk(1,"","Ng­êi cña gi¸o ph¸i kh¸c t¹i sao l¹i ®Õn ®©y nhê cËy ta?")
	end
	if(GetCash() >= 50000) then						
		Msg2Faction("emei", GetName().." tõ h«m nay trë vÒ l¹i s­ m«n, mong c¸c huynh ®Ö chØ gi¸o!", GetName()) ----------
		Talk(1,"","RÊt tèt! Hoan nghªnh ng­¬i trë l¹i Nga my!")
		Pay(50000)
		SetTask(5995, 0)
		SetFaction("emei") ------------------
		SetCamp(1) -------------------
		SetCurCamp(1) ----------------------
		-- SetRank(74) -------------------
		AddNote("§· quay trë l¹i Nga my. ")
	else
		Talk(2,"","§Ó vÒ l¹i s­ m«n, ng­¬i cÇn ®em theo 5 v¹n l­îng, sau ®Õ h·y ®Õn gÆp ta!","Xin lçi! §Ó ta kiÓm tra l¹i.")
	end
end
...
function bil4i3nXS()
	...
	if GetLevel()<60 then
			Talk(1,"","Vâ c«ng th× cã nh­ng thùc lùc ch­a ®ñ lµm sao xuèng nói ®©y!.LuyÖn ®Õn cÊp 60 råi h·y t×m ta ")
			return
	end
	Msg2Faction("emei", GetName().." tõ h«m nay rêi khái s­ m«n, xuèng nói hµnh tÈu giang hå, mong c¸c huynh ®Ö chØ gi¸o!", GetName()) -----------
	Talk(1,"","Tèt l¾m! Hµnh tÈu giang hå nhí gióp ®ì nh÷ng ng­êi khèn khã!")
	SetTask(5995, 5) ------------
	AddNote("Rêi khái Nga my, tiÕp tôc hµnh tÈu giang hå. ") -------------
	Msg2Player("Ban ®· rêi khái Nga my, tiÕp tôc hµnh tÈu giang hå. ") -------------
	SetFaction("")
	-- SetFaction("emei")
	SetCamp(4)
	SetCurCamp(4)
...
function Uworld125_finish()
	if ((GetTask(1) >= 70*256) and (GetTask(1) ~= 75*256)) or
		(GetTask(125) == 40 and GetTask(5894) == 1) then
		if GetLastFactionNumber() == 4 then
			...
			SetTask(5894,255)  --Ñ§µÃ¼¼ÄÜµÄÉèÖÃ±äÁ¿255
		end
	else
