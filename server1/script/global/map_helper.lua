...
function main(sel)
	...
	tbDailog.szTitleMsg = "<npc><#>Tuy kh«ng ph¶i lµ ng­êi trong gianghå, nh­ng hµng ngµy ta thÊy c¸c ng­¬i qua l¹i rÊt<enter>nhiÒu lÇn. Kh«ng ph¶i kho¸c l¸c v× hoµn c¶nh c¸c ng­¬ita hiÓu rÊt râ. "
...
function want2cd_signmap()
	if ( GetLevel() < CD_LEVEL_LIMIT ) then
		Say("¸? Ng­¬i còng muèn tham gia chiÕn dÞch thñ thµnh µh? §¼ng cÊp ch­a ®ñ ®©u"..CD_LEVEL_LIMIT.."CÊp, tµi nghÖ nh­ ng­¬i ®©y gióp kh«ng ®­îc g× ®©u, cã thÓ mÊt m¹ng nh­ ch¬i n÷a, th«i ë l¹i thµnh nµy ®i!", 0)
		return
	end
	Say("Ng­¬i muèn ®i gi÷ thµnh µh? Muèn l¾m µh? ói! Ng­¬i vÜ®¹i thËt ®ã! Nh­ng nghe ®©u t×nh huèng cÊp b¸ch l¾m<enter>th× ph¶i! Trong thµnh chØ cßn thñ ®­îc 1 tiÕng n÷a <enter>th«i, nÕu trong 1 tiÕng kh«ng kÝch lui ®­îc qu©n ®Þchth× thÊt thñ råi! Cßn muèn tham gia chØ cã t­íng qu©nmíi quyÕt ®Þnh ®­îc, ta cã thÓ dÉn ng­¬i qua ®ã.", 2, "Ta biÕt råi! Mau dÉn ta qua ®ã ®i!/sure2cd_signmap", "§Ó ta chuÈn bÞ ®·!/no")
end
