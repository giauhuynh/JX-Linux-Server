...
function main()
	...
	if ((UTask_world20 == 0) and (GetLevel() >= 2)) then		-- ÈÎÎñÆô¶¯
		Say("Bè mÑ ®· s¾p xÕp h«n sù cho cho em, nh­ng nhµ em nghÌo qu¸, ®Õn mét mãn cña håi m«n cho ra d¸ng còng kh«ng cã! GÇn ®©y trong trÊn xuÊt hiÖn mét ng­êi th­¬ng nh©n tõ ph­¬ng xa ®Õn, anh/chŞ cã thÓ hái gióp em xem cã mãn nµo gi¸ rÎ mét chót kh«ng? ", 2, "Gióp/yes", "Kh«ng gióp/no")
	elseif(UTask_world20 == 1) then
		if(HaveItem(181) == 1) then
			DelItem(181)
			SetTask(48, 3)			-- ÈÎÎñÔİÊ±¸ÄÎª²»¿ÉÖØ¸´İê³É
			Talk(1,"","A! §«i b«ng tai nµy ®Ñp qu¸, cã lÏ ®· tèn cña anh/chŞ kh«ng İt tiÒn! §©y lµ chiÕc kh¨n em tù lµm, tÆng cho anh/chi!")
			AddNote("§­a b«ng tai cho A Ph­¬ng hoµn thµnh nhiÖm vô. ")
			Msg2Player("§­a b«ng tai cho A Ph­¬ng hoµn thµnh nhiÖm vô.")
			p1,p2,p3,p4,p5,p6 = RndItemProp(6,10)
			if(GetSex() == 0) then
				AddItem(0, 7, 6, 1, random(0,4), GetLucky(), p1,p2,p3,p4,p5,p6)
				Msg2Player("B¹n nhËn ®­îc mét chiÕc kh¨n")
			else
				AddItem(0, 7, 8, 1, random(0,4), GetLucky(), p1,p2,p3,p4,p5,p6)
				Msg2Player("B¹n nhËn ®­îc mét chiÕc kh¨n")
			end
			AddRepute(6)		-- ¼ÓÉùİû6µã
			Msg2Player("Thanh thÕ giang hå cña b¹n ®­îc t¨ng 6 ®iÓm")
		else
			Talk(1,"","Ng­êi th­¬ng nh©n ®ã ë trong thŞ trÊn, lµm phiÒn anh/chŞ hái gióp em víi. ")
		end
	else				-- ·ÇÈÎÎñ¶Ô»°
