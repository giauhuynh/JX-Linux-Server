...
function main()
	...
	elseif(UTask_world18 == 1) then		-- ÈÎÎñİê³É
		if(HaveItem(180) == 1) then
			Talk(1,"","å! §©y lµ thuèc Tú Bµ hoµn ­? Nh­ vËy bÖnh t×nh cña bè ch¸u nhÊt ®Şnh sÏ khái. C«/chó thËt lµ mét ng­êi tèt, C«/chó h·y nhËn lÊy ®«i giÇy nµy, cña mÑ ch¸u lµm ®Êy. ")
			DelItem(180)
			SetTask(46, 2)
			AddNote("§­a Tú Bµ Hoµn cho TiÓu Ng­ hoµn thµnh nhiÖm vô.")
			Msg2Player("§­a Tú Bµ Hoµn cho TiÓu Ng­ hoµn thµnh nhiÖm vô. ")
			p1,p2,p3,p4,p5,p6 = RndItemProp(6,10)
			if(GetSex() == 0) then
				AddItem(0, 5, 0, 1, random(0,4), GetLucky(), p1,p2,p3,p4,p5,p6)
				Msg2Player("B¹n ®­îc tÆng mét ®«i giµy")
			else
				AddItem(0, 5, 2, 1, random(0,4), GetLucky(), p1,p2,p3,p4,p5,p6)
				Msg2Player("B¹n ®­îc tÆng mét ®«i giµy")
			end
			AddRepute(5)		-- ¼ÓÉùİû6µã
		...
		if (random(0,1) == 0) then
			Talk(1,"","Bè ch¸u lµ ng­ d©n trªn §éng §×nh hå, mçi ngµy ®Òu ph¶i ra hå ®¸nh c¸, nÕu h«m nµo gÆp sãng to giã lín, cã khi mÊy ngµy liÒn kh«ng thÓ vÒ nhµ. ")
		else
...
function yes()
	...
	SetTask(46, 1)
	Msg2Player("TiÕp nhËn nhiÖm vô: Bè cña TiÓu Ng­ bŞ bÖnh, gióp TiÓu Ng­ ®i mua thuèc Tú Bµ Hoµn ®Ó ch÷a bÖnh. ")
	AddNote("TiÕp nhËn nhiÖm vô: Bè cña TiÓu Ng­ bŞ bÖnh, gióp TiÓu Ng­ ®i mua thuèc Tú Bµ Hoµn ®Ó ch÷a bÖnh. ")
end;

function no()
	Talk(1,"","Ch¸u ph¶i ë ®©y ®Ó tiÕp tôc chê nh÷ng ng­êi h¶o t©m")
end;
