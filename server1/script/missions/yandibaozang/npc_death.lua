...
function YDBZ_award(index,ntype,nplayindex)
	...
	bilActivity_DeathLastNpcAiYandiBaozang(world, x, y, nplayindex)
...
function YDBZ_award_finalboss(camp)
	...
	for nj = 1, 10 do
		...
		if (pidx > 0) then
			...
			bilActivity_EnYandiBaozang()
			
			SetTask(5975, GetTask(5975) + 1)
			Ladder_NewLadder(10233, GetName(), GetTask(5975), 1);
		end
		if (idx == 0) then
			break;
		end;
	...
	elseif nband >= 1 and nband <= 30 then	--Ç°10Ð¡¹ØµÄboss¹Ö		
			...
			if npcway == 10 then
				if GetMissionV(YDNZ_STATE_SIGN) ~= 0 then
					Msg2MSAll(YDBZ_MISSION_MATCH,"§· b­íc vµo giai ®o¹n tranh ®o¹t, cöa ¶i ®· ®ãng.")
					return 
					end
				end
