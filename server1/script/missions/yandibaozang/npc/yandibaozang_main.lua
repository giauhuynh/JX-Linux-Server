...
function main()
	if bilNPCBinhBinhCoNuong ~= 1 then
		return Talk(1, "", "<color=Orange>B×nh B×nh C« N­¬ng: <color>H«m nay t¹i h¹ bÞ c¶m phong hµn, h«m sau c¸c h¹ l¹i ®Õn vËy!")	
	end
	local tbarytalk = {
	"<dec><npc>C¸ch ch¬i v­ît ¶i míi, néi dung <enter>míi, boss míi, b¶n ®å míi, v« vµn kú tr©n dÞ b¶o, rÊt nhiÒu c¸c vËt phÈm cã gi¸ trÞ kh¸c chØ cã t¹i b¶o tµngviªm ®Õ. C¸c h¹ ®· chuÈn bÞ ch­a?",
	"Xem t×nh h×nh tæ ®éi/YDBZ_view_player",
	"Thêi gian b¸o danh tham gia b¶o tµng viªm ®Õ/YDBZ_about",
...
function YDBZ_checkLimit2()
	...
	if GetTeamSize() ~= 0 then
		Say("CÇn ph¶i trë vÒ ë cöa vµo b¶o tµng viªm ®Õ cña tæ ®éihiÖn giê kh«ng thÓ tæ ®éi ®­îc",0)
		return 1
	end
...
function YDBZ_checkLimit()
	...
	if (IsCaptain() ~= 1) then
		local tbarytalk ={
			"<dec><npc>Tham gia ho¹t ®éng v­ît ¶i viªm ®Õb¶o tµng cÇn ph¶i do <color=green>®éi tr­ëng<color> b¸o danh míi vµo ®­îc.",
			"BiÕt råi/NoChoice",
		}
		CreateTaskSay(tbarytalk)
		return 1
	...
	if (GetTeamSize() < YDBZ_TEAM_COUNT_LIMIT or GetTeamSize() > YDBZ_TEAM_COUNT_MAXLIMIT) then
		local tbarytalk ={
			"<dec><npc>Tæ ®éi tham gia b¶o tang viªm ®Õ  cÇn Ýt nhÊt <color=red>"..YDBZ_TEAM_COUNT_LIMIT.."<color> ng­êi vµ kh«ng thÓ v­ît qu¸ <color=red>"..YDBZ_TEAM_COUNT_MAXLIMIT.."<color>. Tr­íc m¾tsè ng­êi trong tæ ®éi kh«ng phï hîp yªu cÇu.",
			"BiÕt råi/NoChoice",
		}
		CreateTaskSay(tbarytalk)
		return 1
...
--		end
		...
		-- Thay ®æi c¸ch b¸o danh Viªm ®Õ, lÇn ®Çu tiªn trong ngµy cã thÓ tham gia miÔn phÝ - Modified By bil4i3n - 20161118
			if bil.TaskDate(5933, 0, 5932) > 0 then
				if ((numzimu+numbeibao) < YDBZ_LIMIT_ITEM[2] and (ndnumzimu+ndnumbeibao) < YDBZ_LIMIT_DOUBEL_ITEM[2]) then
					noitem = noitem + 1
					tbstr[2][noitem] = "<color=yellow>"..GetName().."<color>"
				end
		...
		if GetTask(5932) >= 2 then
			bil_nLimitDay = bil_nLimitDay + 1
			tbstr[5][bil_nLimitDay] = "<color=yellow>"..GetName().."<color>"
		end
	end
	...
	if noitem > 0 then
		...
		szstr = szstr .. format("Ng­êi ch¬i %s ®· <color=white>hÕt l­ît miÔn phÝ<color> vµ kh«ng mang theo %s.",szstr1,YDBZ_LIMIT_ITEM[3])
	end
	if nocount > 0 then
	...
	if (nAcc > 0) then
		...
		szstr = szstr .. format("Ng­êi ch¬i %s ®· <color=white>hÕt l­ît miÔn phÝ<color> vµ kh«ng mang theo %s.",szstr1,YDBZ_LIMIT_DOUBEL_ITEM[3])
	end
	if (bil_nLimitDay > 0) then
		local szstr1 = tbstr[5][1]
		for i = 2, bil_nLimitDay do 
			szstr1 = szstr1 .. ", " .. tbstr[5][i]
		end
		szstr = szstr .. format("Ng­êi ch¬i %s ®· <color=white>hÕt l­ît tham gia trong ngµy<color>.",szstr1)
	end
	
	
	if noplaylv + noitem + nAcc + nocount + bil_nLimitDay > 0 then
		Msg2Team(szstr);
		Say(szstr,0)
		return 1 	
	end
...
function YDBZ_want_play()
	...
	if (startboat == 1) then
		...
		CreateTaskSay(tbarytalk)
			...
			"<dec><npc>Khu vùc thµnh nµy "..strlevel.." tiÕn vµo giai ®o¹n b¸o danh, Mçi ngµy miÔn phÝ 1 lÇn tõ lÇn 2 trë ®i mçi thµnh viªn cÇn ph¶i nép <color=red>"..YDBZ_LIMIT_ITEM[2].." c¸i "..YDBZ_LIMIT_ITEM[3].."<color>, ®¹i hiÖp cã muèn tham gia?",
		...
		local tbarytalk ={
			"<dec><npc>Xin lçi, khu vùc thµnh nµy "..strlevel.." HiÖn t¹i kh«ng cã nhiÖm vô b¶o tµng viªm ®Õ nµo cã thÓ tham gia. B¾t ®Çu tõ <color=red>6:00 ®Õn 24:00<color>, xin h·y chó ý th«ng b¸o vµ ®Õn tr­íc 5 phót ®Ó b¸o danh.",
			"BiÕt råi/NoChoice",
		}
		CreateTaskSay(tbarytalk)
		return
...
function YDBZ_about()
	...
	Say(format("Mçi ngµy <color=yellow>%s<color>, h·y ®Õn tr­íc <color=green>5 phót<color> ®Ó b¸o danh ë <color=Orange>B×nh B×nh c« n­¬ng<color>.", "7:00, 11:00, 15:00, 19:00, 23:00"),0)
