...
function RunMission()
	...
	for i = 1 , 100 do 
		...
		--´ËÊ±ÎªGM·¢²¼ÃüÁî
		SetFightState(1)
		PutMessage("ThuyÒn ®· rêi bÕn, 30 phót n÷a sÏ tíi bê b¾c.")
		if (idx == 0) then 
 			break
 		end 
 	end
...
function OnLeave(RoleIndex)
	...
	Msg2MSAll(MISSIONID, GetName().." bÊt h¹nh bá m×nh trªn thuyÒn.")
	SetCreateTeam(1)
	ForbidEnmity(0);
--	SetTaskTemp(200,0);
	SetDeathScript("")
...
function Landing()
	Msg2MSAll(MISSIONID, "¡ì¡¤ ??n b¨º b?c phong l¡§ng ?¨¦.")
	...
	if (nDate >= jf0904_act_dateS and nDate < jf0904_act_dateE) then
		local szNews = "C¸c vŞ cao thñ ®· ®em thñy tÆc ®µu lÜnh tiªu diÖt. h·y mau nhanh ch©n ®Ó ta cßn ®i tiÕp!";
