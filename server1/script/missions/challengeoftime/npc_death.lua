...
function drop_item(index, count)
	...
	if (count ~= 0) then
		for i = 1, count do
			-- µôÂäÎå»¨
			DropItem(world, x, y, -1, 1, 2, 0, 5, 0, 0, 0, 0, 0, 0, 0, 0); -- ng? hoa ngäc lé hoµn
		end
	end
	-- µôÂäÀñÆ·ºĞ
	if ( random(1,100) <= 5 ) then
		DropItem(world, x, y, -1, 6, 1, 1392, 0, 0, 0, 0, 0, 0, 0, 0, 0); -- lÔ vËt v­ît ¶i
	end
end
...
function award_batch(batch, npc_index)
	...
	while (1) do
		...
		if (player > 0) then
			...
			PlayerIndex = player;
				_EVENT:Activity_VA(batch, n_level)
			PlayerIndex = OldPlayerIndex;
...
function success(npc_index)
	...
	broadcast(GetMissionS(VARS_TEAM_NAME) .. " §éi ngò ®· th¾ng lîi hoµn thµnh nhiÖm vô ®óng thêi gian, ®· dïng " .. floor(time / 60) .. " phót " .. mod(time, 60) .. " gi©y! Tr­íc ®ã "..laddertime.." gi©y");
...
function batch_finish(index)
			local x, y, world = GetNpcPos(index);
			bilActivity_DeathLastNpcChallengeOfTime(world, x, y)
