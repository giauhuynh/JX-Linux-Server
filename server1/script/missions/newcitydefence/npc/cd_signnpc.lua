...
function main()
	...
	if ( state ~= 1 and state ~= 2 ) then
		Say(CD_SIGNNPCNAME..": ChiÕn tr­êng thñ thµnh vÉn ch­a b¾t ®Çu thêi gian 8h30 - 12h30 - 16h30 - 18h30 - 22h30 vµ 2h30 s¸ng. !", 0)
...
--	elseif ( nday ~= GetTask( TASKID_FIRE_DAY ) ) then	--ÔÊÐí±¨Ãû
	if ( GetTask( TASKID_FIRE_KEY ) ~= randkey or  GetTask( TASKID_FIRE_DAY ) ~= nday) then	--ÔÊÐí±¨Ãû
		Say(CD_SIGNNPCNAME..": Lý NhÞ! Tªn tiÓu tö nhµ ng­¬i mau ®i kªu TÝn Sö l¹i ®©y, qu©n ®Þch l¹i ®¸nh ®Õn råi! ¥? Ng­¬i lµ ai? B©y giê lµ lóc nµo? Cã ng­êi ®ang tÊn c«ng thµnh cña ta, ng­¬i cßn kÐo dµi qu©n c¬ sÏ bÞ chÐm ®Çu biÕt kh«ng?", 3, "T­íng qu©n! Ta ®Õn gióp thñ thµnh!/#want2joincd1("..defencemap..")", "NhËn thuèc lag tr­íc khi tham chiÕn/thuocphonghoa", "Th«i ta kh«ng muèn chÕt oan uæng/laughtoleave")
		cd_clear_lastsumexp();		--Çå³ýÉÏ´Î¼ÆËãµÄÀÛ¼Æ¾­ÑéÖµ
	else
		Say(CD_SIGNNPCNAME..": ¥? Ch¼ng ph¶i ng­¬i ®· vµo chiÕn tr­êng sao? Hay lµ ta bi hoa m¾t? enter> Th«i ®­îc råi! Ng­¬i chÈun bÞ vµo trong ®ã ®i!", 3, "Ta muèn gia nhËp/#sure2joincd("..defencemap..")", "NhËn thuèc lag tr­íc khi tham chiÕn/thuocphonghoa", "Ta chØ ghÐ qua xem/OnCancel")
	end
end

function sure2joincd(defencemap)
Msg2SubWorld("<color=green>§¹i hiÖp "..GetName().."<color=cyan> t¹i h¹ ®· vµo chiÕn tr­êng thñ thµnh t×m m¶nh cå sè 1 anh em vµo cïng phô t¹i h¹ mét tay nµo <pic=1>")
	local oldSubWorld = SubWorld;
	local signmap = SubWorldIdx2ID( SubWorld )
	...
	if ( GetTask( TASKID_FIRE_KEY ) ~= randkey or  GetTask( TASKID_FIRE_DAY ) ~= nday) then
		if (GetCash() >= 0) then
			...
			Say(CD_SIGNNPCNAME..": Sao thÕ, ng­¬i kh«ng mang theo"..floor(FIRE_JOINUP_FEE / 1000).." v¹n l­îng, ta kh«ng thÓ cho ng­¬i vµo. H·y chuÈn bÞ tiÒn råi h·y ®Õn t×m ta!", 0);
			return -1;
		end;
	end;
	cd_setsign_levelexp();
...
function want2joincd2(defencemap)
	...
	SubWorld = oldSubWorld
	Say(CD_SIGNNPCNAME..": §­îc råi! Kh«ng cÇn nãi nhiÒu, nãi chung ta c¶m thÊy lai lÞch cña bän hä rÊt kh¶ nghi, trong lóc chiÕn ®Êu nÕu ng­¬i nhÆt ®­îc th­ hµm th× h·y ®em ®Õn ta xem, ta sÏ träng th­ëng! Sè ng­êi tham gia thñ thµnh lµ<color=yellow>"..cd_membercount.."<color> ng­êi. Tr­íc tiªn ph¶i nép 1 l­îng phÝ b¸o danh, chuÈn bÞ s½n sµng ch­a?", 2, "§óng vËy!/#sure2joincd("..defencemap..")", "§Ó ta chuÈn bÞ ®·!/OnCancel")
end
...
tbAwardTemplet:GiveAwardByList({{szName="Thuèc Lag",tbProp={6,1,190},nCount=50,nExpiredTime=70, nBindState = -2},}, "test", 1);
tbAwardTemplet:GiveAwardByList({{szName="Thuèc Lag",tbProp={6,1,157},nCount=50,nExpiredTime=70, nBindState = -2},}, "test", 1);
end