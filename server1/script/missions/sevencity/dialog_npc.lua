...
local dayw = tonumber(date("%w"))
local state = BattleWorld.m_State
local zstate = state
local hour = tonumber(GetLocalDate("%H"))
local mmin = tonumber(GetLocalDate("%M"))
	if(zstate==0) then
		checkstate = "ch­a cho phep b¸o danh";
	elseif(zstate==1) then
		checkstate = "cho phep b¸o danh";
	elseif(zstate==2) then
		checkstate = "kÕt thóc b¸o danh";
	elseif(zstate==3) then
		checkstate = "®ang trong qu¸ tr×nh chuÈn bi.";
	elseif(zstate==4) then
		checkstate = "b¾t ®Çu c«ng thµnh";
	elseif(zstate==5) then
		checkstate = "kÕt thóc c«ng thµnh";
end
	Say("§©y lµ n¬i nghÞ sù c«ng thµnh chiÕn, ng­¬i ®Õn cã viÖc g×? <enter><enter><color=red>H«m nay lµ<color>: <color=yellow>Ngµy <color><color=green>"..day.."<color> <color=yellow>Th¸ng <color><color=green>"..mm.."<color> <color=yellow>N¨m<color> <color=green>"..year.."<color> - <color=green>"..hour.."<color> <color=yellow>Giê<color> <color=green>"..mmin.."<color> <color=yellow>Phót<color>.<enter><color=red>BattleWorld.m_State<color>: <color=green>"..state.." - "..checkstate.."<color> <enter><color=red>DayW: <color><color=green>"..dayw.."<color>",
		6,
...
function dlg_signup()
	...
	elseif (state == STATE_STARTSIGNUP) then
		Say("ThÊt thµnh ®¹i chiÕn ®ang vµo giai ®o¹n b¸o danh, ch­ vÞ bang chñ h·y ®¹i diÖn bæn bang ®Ó b¸o danh tham gia, phÝ b¸o danh <color=red>10000 v¹n ng©n l­îng<color> vµ<color=red> 200 khiªu chiÕn lÖnh<color>. \nBang héi chiÕm thµnh kh«ng cÇn b¸o danh vµ cã thÓ trùc tiÕp vµo",
			2,
...
function dlg_query()
	...
	if nWeekDay ~= 4  or nHour < 2145 or nHour > 2350 then
