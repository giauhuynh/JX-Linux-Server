...
function Bil_VietFactionWithCnInput(FactionCn)
	-- for i = 1, 11 do
		...
		-- end
	-- end
	return Bil_FactionVietvsCn[FactionCn+1][2]
end
...
function wlls_addplayer_combat(tbLGs, nOrgCamp, nEmyCamp, nNewCamp, tbNewPos)
	...
	for _, nIdx in tbLGs[nEmyCamp].tbPlayer do
		PlayerIndex = nIdx
		szMsg	= szMsg..format("\n<color=white>%16s <color=green>%3d cÊp <color=yellow>%s", GetName(), GetLevel(), Bil_VietFactionWithCnInput(GetLastFactionNumber()))
	end
	Msg2MSGroup(WLLS_MSID_COMBAT, szMsg, nNewCamp)
...
function OnTimer()
	...
	if (n_timer <= 0) then	--×¼±¸½áÊø£¬¿ªÊ¼±ÈÈü
		...
		local Bil_ReportSec = ((n_timer*WLLS_TIMER_PREP_FREQ/60)*60)
		if Bil_ReportSec == 30 then
			Msg2SubWorld("Thêi gian b¸o danh Liªn §Êu cßn <color=yellow>"..Bil_ReportSec.."<color> gi©y")
		elseif Bil_ReportSec == 20 then
			Msg2SubWorld("Thêi gian b¸o danh Liªn §Êu cßn <color=yellow>"..Bil_ReportSec.."<color> gi©y")
		elseif Bil_ReportSec == 10 then
			Msg2SubWorld("Thêi gian b¸o danh Liªn §Êu cßn <color=yellow>"..Bil_ReportSec.."<color> gi©y")
		end
