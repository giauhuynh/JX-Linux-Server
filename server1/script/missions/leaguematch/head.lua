...
function wlls_award_pl_exp(n_rate, n_alevel)
	...
	local Bil_n_exp
	if n_rate == 3 then
		-- Bil_n_exp = 3000000 -- th¾ng nhËn ®c 3tr Exp
		Bil_n_exp = 1000000 -- th¾ng nhËn ®c 3tr Exp
		AddSkillState(451, 1, 0, 2*60*18)
	elseif n_rate == 2 then
		-- Bil_n_exp = 1500000 -- hßa nhËn ®c 1.5tr Exp
		Bil_n_exp = 800000 -- hßa nhËn ®c 1.5tr Exp
	elseif n_rate == 1 then
		-- Bil_n_exp = 1500000 -- thua nhËn ®c 1.5tr Exp
		Bil_n_exp = 500000 -- thua nhËn ®c 1.5tr Exp
	end
	wlls_add_exp(Bil_n_exp)
	local str = "NhËn ®­îc phÇn th­ëng thi ®Êu, <color=yellow>"..Bil_n_exp.." <color>®iÓm kinh nghiÖm"
	wlls_award_log(str)
	Msg2Player(str)
end
...
function wlls_GetAddPoint(nLevel, nWin, nTie)
	return (nWin * 5 * nLevel + nTie * 2 * nLevel) + 5 -- BIl_ mçi trËn th¾ng sÏ ®c 15 ®iÓm, thua ko c? ®iÓm vµ ®Êu tr­êng s¬ - trung cÊp th¾ng sÏ ®c 10 ®iÓm
end
