...
function main(sel)
	...
	if ((Uworld127 < 10) and (GetLevel() >= 90) and (Uworld30 == 127) and (GetReputeLevel(GetRepute()) >= 5) and (GetFaction() ~="tianren" )) or
		(GetLevel() >= 90 and GetCamp() == 4 and GetTask(5891) == 0) then   --·ûºÏÈÎÎñÌõ¼ş
...
function U127_finish()
	if(GetTask(127) == 110) or (GetTask(5891) == 1 ) then
		if GetLastFactionNumber() == 7 then
			...
			Msg2Player("Häc xong vâ c«ng V©n Long Kİch, Thiªn ngo¹i l­u tinh, NhiÕp hån lo¹n t©m cña Thiªn nhÉn. ")
		end
      SetTask(127,255)
      SetTask(5891,255)
   else
