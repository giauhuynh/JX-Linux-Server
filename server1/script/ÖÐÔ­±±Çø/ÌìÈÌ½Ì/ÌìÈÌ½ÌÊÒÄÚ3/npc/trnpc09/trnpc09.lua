...
function oldentence()
	...
	if GetTask(5995) == 7 then -- thien nhan
	...
	-------------------------------------------------------------------------
	if bilFunc_ChuongMonThapDaiPhai(8, GetLevel(), GetLastFactionNumber(), GetFaction(), GetName()) then
	
		local a, b = bilFunc_ChuongMonThapDaiPhai(8, GetLevel(), GetLastFactionNumber(), GetFaction(), GetName())
	...
	tinsert(tbDes, "§Ö tö muèn b¸i nghÖ ®Çu s­ vµo Thiªn nhÉn!/#bilPartner2Sect(8)")
...
function bil4i3nXS_Return()
	local _, nTongID = GetTong();
	if GetTask(5995) ~= 7 and nTongID ~= 0 then ----------
		return Talk(1,"","Ng­êi cña gi¸o ph¸i kh¸c t¹i sao l¹i ®Õn ®©y nhê cËy ta?")
	end
	if(GetCash() >= 50000) then						
		Msg2Faction("tianren", GetName().." tõ h«m nay trë vÒ l¹i s­ m«n, mong c¸c huynh ®Ö chØ gi¸o!", GetName()) ----------
		Talk(1,"","RÊt tèt! Hoan nghªnh ng­¬i trë l¹i Thiªn nhÉn!")
		Pay(50000)
		SetTask(5995, 0)
		SetFaction("tianren") ------------------
		SetCamp(2) -------------------
		SetCurCamp(2) ----------------------
		-- SetRank(81) -------------------
		AddNote("§· quay trë l¹i Thiªn nhÉn. ")
	else
...
function bil4i3nXS()
	...
	Msg2Faction("tianren", GetName().." tõ h«m nay rêi khái s­ m«n, xuèng nói hµnh tÈu giang hå, mong c¸c huynh ®Ö chØ gi¸o!", GetName()) -----------
	Talk(1,"","Tèt l¾m! Hµnh tÈu giang hå nhí gióp ®ì nh÷ng ng­êi khèn khã!")
	SetTask(5995, 7) ------------
	AddNote("Rêi khái Thiªn nhÉn, tiÕp tôc hµnh tÈu giang hå. ") -------------
	Msg2Player("B¹n ®· rêi khái Thiªn nhÉn, tiÕp tôc hµnh tÈu giang hå. ") -------------
	SetFaction("")
	-- SetFaction("tianren")
	SetCamp(4)
	SetCurCamp(4)
