...
function main()
	...
	-- end
	-- if GetTask(5995) == 2 then  -- thieu lam
	...
	-- tbDailog:AddOptEntry("§Ö tö muèn b¸i nghÖ ®Çu s­ vµo ThiÕu L©m!", bilPartner2Sect, {1})
	...
	-------------------------------------------------------------------------
	if bilFunc_ChuongMonThapDaiPhai(1, GetLevel(), GetLastFactionNumber(), GetFaction(), GetName()) then
	
		local a, b = bilFunc_ChuongMonThapDaiPhai(1, GetLevel(), GetLastFactionNumber(), GetFaction(), GetName())
	...
	tinsert(tbDes, "§Ö tö muèn b¸i nghÖ ®Çu s­ vµo ThiÕu L©m ph¸i!!/#bilPartner2Sect(1)")
...
function bil4i3nXS_Return()
	...
	if GetTask(5995) ~= 2 and nTongID ~= 0 then ----------
		return Talk(1,"","Ng­êi cña gi¸o ph¸i kh¸c t¹i sao l¹i ®Õn ®©y nhê cËy ta?")
	end
	if(GetCash() >= 50000) then						
		Msg2Faction("shaolin", GetName().." tõ h«m nay trë vÒ l¹i s­ m«n, mong c¸c huynh ®Ö chØ gi¸o!", GetName()) ---------------
		Talk(1,"","RÊt tèt! Hoan nghªnh ng­¬i trë l¹i ThiÕu L©m!")
		Pay(50000)
		SetTask(5995, 0)
		SetFaction("shaolin") ------------------
		SetCamp(1) -------------------
		SetCurCamp(1) ----------------------
		-- SetRank(72) -------------------
		AddNote("§· quay trë l¹i ThiÕu L©m")
	else
...
function bil4i3nXS()
	...
	Msg2Faction("shaolin", GetName().." tõ h«m nay rêi khái s­ m«n, xuèng nói hµnh tÈu giang hå, mong c¸c huynh ®Ö chØ gi¸o!", GetName()) -------------
	Talk(1,"","Tèt l¾m! Hµnh tÈu giang hå nhí gióp ®ì nh÷ng ng­êi khèn khã!")
	SetTask(5995, 2) ------------
	AddNote("Rêi khái ThiÕu L©m, tiÕp tôc hµnh tÈu giang hå. ")
	Msg2Player("Ban ®· rêi khái ThiÕu L©m, tiÕp tôc hµnh tÈu giang hå. ")
	SetFaction("")
	-- SetFaction("shaolin")
	SetCamp(4)
	SetCurCamp(4)
