...
function main(sel) 
  	...
  	if ((UTask_world38 == 0) and (GetLevel() >= 6)) then		-- ĞèÒª6¼¶ÒÔÉÏ²ÅÄÜ×öÈÎÎñ
  		Talk(3, "select", "TiÓu c« n­¬ng t¹i sao l¹i khãc lãc th¶m th­¬ng vËy?", "Gia Gia m¾c bÖnh råi! Nh­ng nhµ ta l¹i kh«ng cã tiÒn mua thuèc.", "Muèn mua thuèc g×?")
  	elseif (UTask_world38 == 1) then
		Talk(1,"","C¶m ¬n ng­¬i! Thuèc cña «ng néi ta tr­íc ®©y lµ do Chñ d­îc ®iÕm bèc.")
  		...
  		if (HaveItem(186) == 1) then
  			...
  			if (random(0,3) == 0) then				-- 25%µÄ»úÂÊµÃ¶ş¼¶½äÖ¸
				p1,p2,p3,p4,p5,p6 = RndItemProp(6,15)
  				...
  				Msg2Player("NhËn ®­îc 1 chiÕc nhÉn")
  			else
  			...
  			Say("DHT027Äã²»Ğ¡ĞÄÅª¶ª÷êÏã»¢¹Ç¸àÁË£¬Èç¹ûÏëÍê³ÉÈÎÎñ£¬ĞèÒªÓĞ÷êÏã»¢¹Ç¸à.", 2, "ÎÒÈ¥ÕÒÇ®/find", "ÕÒÒ©µêÀÏ°å×öĞÂÒ©/redo")
  		end		
	elseif ((UTask_world38 > 1) and (UTask_world38 < 10)) then			-- ÈÎÎñÖĞµÄÆÕÍ¨¶Ô»°
		Talk(1,"","C¶m ¬n ng­¬i! Thuèc cña «ng néi t«i tr­íc ®©y kaf do Chñ d­îc ®iÕm bèc.")
...
function select()
	Say("Ta còng kh«ng biÕt lµ thuèc g×, chİnh lµ tr­íc ®©y Chñ d­îc ®iÕm «ng ta cho.", 2, "Gióp lµm thuèc/yes", "Kh«ng liªn quan ®Õn viÖc cña ta/no")
end;

function yes()
	Talk(1,"","C¶m ¬n ng­¬i! Thuèc cña «ng néi t«i tr­íc ®©y kaf do Chñ d­îc ®iÕm bèc.")
	SetTask(66,1)
	AddNote("NhËn nhiÖm vô: Gióp TiÓu Lan t×m thuèc cho «ng c« Êy. ")
	Msg2Player("NhËn nhiÖm vô: Gióp TiÓu Lan t×m thuèc cho «ng c« Êy. ")
end;
...
function redo()
Talk(1,"","DHT034µ½Ò©µêÈ¡Ò©!")
SetTask(66, 2)
SetTask(67, 0)
AddNote("<color=red> DHT035ÒòÎª÷êÏã»¢¹Ç¸à¶ªÁË£¬ÇëÈ¥ÕÒÒ©µêÀÏ°å×öÒ©. <color> ")
Msg2Player("ÒòÎª÷êÏã»¢¹Ç¸à¶ªÁË£¬ÇëÈ¥ÕÒÒ©µêÀÏ°å×öÒ©. ")
end;
