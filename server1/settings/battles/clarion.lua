...
function EatMedicine()	-- Í¨¹ýÅÐ¶ÏÕ½³¡µÄµÈ¼¶ºÍÍæ¼ÒµÄÕóÓªÀ´¼ÓNPC
	...
	for j = 1, getn(tbPK_MAP) do
		if ( nMapId == tbPK_MAP[j] ) then
			Msg2Player("Khu b¶n ®å nµy kh«ng thÓ sö dông ®¹o cô nµy.");
			return -1
		end
	end
	...
	for j = 1, getn(tbCD_MAP) do
		if ( nMapId == tbCD_MAP[j] ) then
			Msg2Player("§¹o nµy cô chØ cã thÓ ë chiÕn tr­êng tèng kim sö dông");
			return -1
		end
	end
	if (GetCurCamp() == 1) then
		if (GetMissionV(MS_CALLNPCCOUNT_S) >= MAX_CALLNPCCOUNT) then
			Msg2Player("Tr­íc m¾t chiÕn tr­êng sö dông kÌn hiÖu sè l­îng ®· ®¹t tíi th­îng h¹n, kh«ng thÓ sö dông n÷a.")
			return -1
		else
	...
	if ( GetLevel() >= 40 and GetLevel() <= 79) then	-- ³õ¼¶Õ½³¡
		if( GetCurCamp() == 1) then		-- ÕÙ»½ºÍÍæ¼ÒÍ¬Ò»ÕóÓªµÄNPC
			CallSjNpc( 682, 50, W, X, Y, "SÜ binh Tèng qu©n");
		elseif( GetCurCamp() == 2) then
			CallSjNpc( 688, 50, W, X, Y, "Kim qu©n sÜ binh");
		end
	elseif ( GetLevel() >= 80 and GetLevel() <= 119) then	-- ÖÐ¼¶Õ½³¡
		if( GetCurCamp() == 1) then
			CallSjNpc( 682, 70, W, X, Y, "SÜ binh Tèng qu©n");
		elseif( GetCurCamp() == 2) then
			CallSjNpc( 688, 70, W, X, Y, "Kim qu©n sÜ binh");
		end
	else	-- ¸ß¼¶Õ½³¡
		if( GetCurCamp() == 1) then
			CallSjNpc( 682, 90, W, X, Y, "SÜ binh Tèng qu©n");
		elseif( GetCurCamp() == 2) then
			CallSjNpc( 688, 90, W, X, Y, "Kim qu©n sÜ binh");
		end
	end
end	
